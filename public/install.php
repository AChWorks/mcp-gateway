    ...
    <div class="card">
        ...
        <?php else: ?>
            ...
            <form method="post" autocomplete="off">
                ...
                <?php if (isset($errors['app_url'])): ?><div class="error"><?= installerEscape($errors['app_url']) ?></div><?php endif; ?>

                <h2>MySQL</h2>
                <label>Host
                    <input name="db_host" required value="<?= installerEscape($values['db_host']) ?>">
                </label>
                <?php if (isset($errors['db_host'])): ?><div class="error"><?= installerEscape($errors['db_host']) ?></div><?php endif; ?>

                <label>Port
                    <input name="db_port" inputmode="numeric" required value="<?= installerEscape($values['db_port']) ?>">
                </label>
                <?php if (isset($errors['db_port'])): ?><div class="error"><?= installerEscape($errors['db_port']) ?></div><?php endif; ?>

                <label>Database
                    <input name="db_database" required value="<?= installerEscape($values['db_database']) ?>">
                </label>
                <?php if (isset($errors['db_database'])): ?><div class="error"><?= installerEscape($errors['db_database']) ?></div><?php endif; ?>

                <label>Username
                    <input name="db_username" required value="<?= installerEscape($values['db_username']) ?>">
                </label>
                <?php if (isset($errors['db_username'])): ?><div class="error"><?= installerEscape($errors['db_username']) ?></div><?php endif; ?>

                <label>Password
                    <input name="db_password" type="password" required autocomplete="new-password">
                </label>
                <?php if (isset($errors['db_password'])): ?><div class="error"><?= installerEscape($errors['db_password']) ?></div><?php endif; ?>

                <h2>Administrator</h2>
                <label>Name
                    <input name="admin_name" required value="<?= installerEscape($values['admin_name']) ?>">
                </label>
                <?php if (isset($errors['admin_name'])): ?><div class="error"><?= installerEscape($errors['admin_name']) ?></div><?php endif; ?>

                <label>Email
                    <input name="admin_email" type="email" required value="<?= installerEscape($values['admin_email']) ?>">
                </label>
                <?php if (isset($errors['admin_email'])): ?><div class="error"><?= installerEscape($errors['admin_email']) ?></div><?php endif; ?>

                <label>Password
                    <input name="admin_password" type="password" required autocomplete="new-password">
                </label>
                <?php if (isset($errors['admin_password'])): ?><div class="error"><?= installerEscape($errors['admin_password']) ?></div><?php endif; ?>

                <label>Confirm password
                    <input name="admin_password_confirmation" type="password" required autocomplete="new-password">
                </label>
                <?php if (isset($errors['admin_password_confirmation'])): ?><div class="error"><?= installerEscape($errors['admin_password_confirmation']) ?></div><?php endif; ?>
