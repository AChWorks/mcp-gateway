...
{
    public function test_current_runtime_accepts_mariadb_and_mysql_with_a_valid_key(): void
    {
        $requirements = new EnvironmentRequirements;
        $key = 'base64:'.base64_encode(str_repeat('k', 32));

        self::assertNotContains(false, $requirements->checks('mariadb', $key, 'AES-256-CBC'), true);
        self::assertNotContains(false, $requirements->checks('mysql', $key, 'AES-256-CBC'), true);
        self::assertFalse($requirements->checks('sqlite', $key, 'AES-256-CBC')['MariaDB or MySQL is the configured database driver']);
        self::assertFalse($requirements->checks('mariadb', null, 'AES-256-CBC')['Valid Laravel encryption key']);
